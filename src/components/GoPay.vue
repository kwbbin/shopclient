<template>
  <div class="GoodsDetail">
   <!--顶部样式--> 
   <div id="header_top"> 
    <div id="top"> 
     <div class="Inside_pages"> 
      <div class="Collection">
       {{getTime()}}欢迎光临旺旺超市！
       <em></em>
       <a href="#">收藏我们</a>
      </div> 
      <div class="hd_top_manu clearfix"> 
       <ul class="clearfix"> 
        <li class="hd_menu_tit zhuce" data-addclass="hd_menu_hover">欢迎光临本店！<span class="red" v-if="user!=''">{{loginName}}</span><span class="red hand" v-if="user!=''" @click="loginOut()">注销</span>
         <router-link to="/login" class="red" v-if="user==''">
          [请登录]
         </router-link> <span v-if="user==''">新用户</span>
         <router-link to="/regist" v-if="user==''" class="red">
          [免费注册]
         </router-link></li> 
        <li class="hd_menu_tit" data-addclass="hd_menu_hover"><a href="#"><router-link to="/">首页</router-link></a></li> 
        <li class="hd_menu_tit" data-addclass="hd_menu_hover"><a href="#"><router-link to="/pay">我的订单</router-link></a></li> 
        <li class="hd_menu_tit" data-addclass="hd_menu_hover"> <a href="#"><router-link to="/ShoppingCart">购物车</router-link></a> </li> 
        <li class="hd_menu_tit" data-addclass="hd_menu_hover"><a href="#">联系我们</a></li> 
        <li class="hd_menu_tit" data-addclass="hd_menu_hover"><a href="#"><router-link to="/user">我的信息</router-link></a></li> 
       </ul> 
      </div> 
     </div> 
    </div> 
    <!--样式--> 
    <!--顶部样式2--> 
    <div id="header " class="header page_style"> 
     <div class="logo">
      <a href="#"><img src="../assets/images/logo.png" /></a>
     </div> 
     <!--可修改图层--> 
     <div class="festival">
      <!-- <a href="#"><img src="../assets/images/logo_yd.png" /></a> -->
     </div> 
     <!--结束图层--> 
     <div class="Search"> 
      <!-- <p><input name="" type="text" class="text" /><input name="" type="submit" value="搜 索" class="Search_btn" /></p>  -->
      <!-- <p class="Words"><a href="#">苹果</a><a href="#">香蕉</a><a href="#">菠萝</a><a href="#">西红柿</a><a href="#">橙子</a><a href="#">苹果</a></p>  -->
     </div> 
     <!--可修改图层2--> 
     <!--<div class="festival1"><a href="#"><img src="images/logo_sd.png" /></a></div>-->
     <!--结束--> 
     <!--购物车样式--> 
     <div class="hd_Shopping_list" id="Shopping_list"> 
      <div class="s_cart">
       <em></em>
       <a href="#"><router-link to="/ShoppingCart">我的购物车</router-link></a> 
       <i class="ci-right">&gt;</i>
      </div> 
     </div> 
    </div> 
   </div> 


    <div class="middle">
      <div style="margin-top:30px;text-align:left;padding-left:20px;">
        <h1>请选择地址</h1>
          <el-radio-group v-model="selectPostion">
            <ul>
              <li v-for="(item, i) in allPosition" style="margin-top:20px;text-align:left;">
                <el-tag  ><el-radio :label="item.id" >{{item.position}}</el-radio></el-tag>
              </li>
            </ul>
            
          </el-radio-group>
      </div>
      
      <h1 style="text-align:left;margin-top:20px;padding-bottom:20px;padding-left:20px;">待付款订单：</h1>
        <el-table
          :data="orders"
          ref="multipleTable"
          style="width: 100%"
          @selection-change="handleSelectionChange">
          <el-table-column
            type="selection"
            width="55">
          </el-table-column>
          <el-table-column
            prop="name"
            label="商品"
            width="180">
          </el-table-column>
          <el-table-column
            prop="goodsDesc"
            label="商品描述"
            width="180">
          </el-table-column>
          <el-table-column
            prop="price"
            label="单价">
          </el-table-column>
          <el-table-column
            prop="num"
            label="数量">
          </el-table-column>
          <el-table-column
            prop="address"
            label="图片"
            width="200">
            <template slot-scope="scope">
              <el-image
                style="width: 100px; height: 100px"
                :src="'http://127.0.0.1:8083/file'+scope.row.imageUrl" 
                >
              </el-image>
            </template>
          </el-table-column>
          <el-table-column
            prop="totalPrice"
            label="总价/元">
          </el-table-column>
          <el-table-column label="操作">
            <template slot-scope="scope">
                <el-button
                size="mini"
                type="danger"
                slot="reference"
                @click="handleDelete(scope.$index, scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div style="margin-top: 20px">
          <el-button @click="toggleSelection(true)">全选</el-button>
          <el-button @click="toggleSelection()">取消选择</el-button>
          <el-button @click="handleDeleteSelect()">删除已选择的</el-button>
        </div>
        
        <div style="text-align:right;margin-top: 20px;">
          <el-button @click="submitOrders()" type="danger">去付款</el-button>
        </div>
    </div>


   <!--底部样式-->
    <div class="ft_footer_service" id="footer">
      <div class="footerbox" >
      <!--底部-->
      <div class="Menu_style ensure ">
        <div class="phone">
          400-3456-333
        </div>
        <div class="icon_en">
        <a href="#" class="icon_link" style="padding-left:0"><img src="../assets/images/footer_icon_31.png" /><span class="left"><h2>退换货</h2>7天无理由退换货</span></a>
        <a href="#" class="icon_link"><img src="../assets/images/footer_icon_33.png" /><span class="left"><h2>正品保障</h2>企业认证产品</span></a>
        <a href="#" class="icon_link"><img src="../assets/images/footer_icon_35.png" /><span class="left"><h2>满包邮</h2>满68元包邮</span></a>
        <a href="#" class="icon_link"><img src="../assets/images/footer_icon_37.png" /><span class="left"><h2>直供产品</h2>厂家直销平台</span></a>
        </div>
      </div>
      </div>
    </div>
    <!--底部样式-->
    <div class="footer">
      <div class="footerbox clearfix">
      <div class="clearfix">
      <div class="left help_link">
        <dl>
      <dt>投保指南</dt>
      <dd><a href="#">保险需求测试</a></dd>
        <dd><a href="#">专题及活动</a></dd>
        <dd><a href="#">挑选保险产品</a> </dd>
        <dd><a href="#">常见问题 </a></dd>
      </dl>
      <dl>
      <dt>保险服务</dt>
      <dd><a href="#">保险需求测试</a></dd>
        <dd><a href="#">专题及活动</a></dd>
        <dd><a href="#">挑选保险产品</a> </dd>
        <dd><a href="#">常见问题 </a></dd>
      </dl>
      <dl>
      <dt>支付方式</dt>
      <dd><a href="#">保险需求测试</a></dd>
        <dd><a href="#">专题及活动</a></dd>
        <dd><a href="#">挑选保险产品</a> </dd>
        <dd><a href="#">常见问题 </a></dd>
      </dl>
      <dl>
      <dt>帮助中心</dt>
      <dd><a href="#">保险需求测试</a></dd>
        <dd><a href="#">专题及活动</a></dd>
        <dd><a href="#">挑选保险产品</a> </dd>
        <dd><a href="#">常见问题 </a></dd>
      </dl>	   
      </div>
      <!--信息内容-->
      </div>

    <div class="text_link">
      <p>
      <a href="#">关于我们</a>｜ <a href="#">公开信息披露</a>｜ <a href="#">加入我们</a>｜ <a href="#">联系我们</a>｜ <a href="#">版权声明</a>｜ <a href="#">隐私声明</a>｜ <a href="#">网站地图</a></p>
      <p>蜀ICP备11017033号 Copyright ©2011 成都福际生物技术有限公司 All Rights Reserved. Technical support:CDDGG Group</p>
      </div>
   </div> 
  </div>
  </div>
</template>

<script>
import '@/assets/css/style.css'
import '@/assets/css/common.css'
import '@/assets/js/common_js.js'
import '@/assets/js/footer.js'
import '@/assets/font/iconfont.css'
import { hostname } from 'os'
import { log } from 'util'

export default {
  name: 'GoodsDetail',
  data () {
    return {
      msg: '',
      welcome: '',
      mydata:'',
      orders:new Array(),
      onlineOrders:new Array(),
      msg:'',
      code:'',
      token:'',
      url:'',
      user:'',
      loginName:'',
      ids:this.$route.query.ids,
      response:'',
      newOrders : new Array(),
      multipleSelection: [],
      tableData:[],
      selectPostion:'',
      allPosition:''
    }
  },
  methods: {
    requestPost:function (str,obj,successfun,failfun){
      this.$axios({
              url:str,
              method:'post',
              data:obj,
              headers: {'token':localStorage.getItem("token")},
          })
          .then(res=>{
              this.response = res.data;
              this.mydata = res.data.data;
              this.msg = res.data.message;
              this.code = res.data.code;
              this.requestNum++; 
              this.token = res.data.token;  
              if(successfun!=null)
              successfun();
          })
          .catch(Error=>{
              if(failfun!=null)
              failfun();
          })
    },
    jqueryAjax:function(url,obj,successfun,failfun){
      $.ajax({
            url: url,//请求路径
            data: JSON.stringify(obj),
            dataType: 'json',
			      async: false,
            contentType:'application/json',
            type: "POST",//GET
            headers: {'token':localStorage.getItem("token")},
            //dataType: "JSON",//需要返回JSON对象(如果Ajax返回的是手动拼接的JSON字符串,需要Key,Value都有引号)
            success: function(resp) {
              successfun(resp);
            },
            error: function(a, b, c) {
                //a,b,c三个参数,具体请参考JQuery API
                // failfun(a,b,c);
            }
        })
    },
    getTime: function(){
      var t = new Date();
      var h = t.getHours();
      if((h>0&&h<=4)||(h<24&&h>17))
        return "晚上好，"
      
      if(h>=5&&h<=11)
        return "上午好，"
      
      if(h>11&&h<14)
        return "中午好，"
      
      if(h>=14&&h<=17)
        return "下午好，"
    },
    loginOut:function(){
      this.requestPost('/user/loginOut');
      localStorage.removeItem("token");
      this.loginName = '';
      this.user = '';
      alert("已注销");
    },
    initData:function(){
      //将onlineOrder保存进本地订单，未选中状态
      //选中状态的订单id将会被提交。
      var This=this;


      // 检验登录
      if(localStorage.getItem('token')!=null){
        this.requestPost('/user/getLoginUser',null,function(){
          if(This.code==0){
              if(This.mydata!= null)
              This.user = This.mydate;
              This.loginName = This.mydata.loginName;
          }else{
            This.alertMessage(This,1,"请先登录")
            This.$router.push('/Login');
            return;
          }
          
        },null)
      }else{
            This.alertMessage(This,1,"请先登录")
        this.$router.push('/Login');
        return;
      }

        //1、读该用户取数据库未付款订单信息，保存在 onlineOrder
        This.jqueryAjax("http://localhost:8081/order/getAllUnpaidOrders",null,function(obj) {
          This.onlineOrders = obj.data;
          //1 读取本地商品，生成订单存入数据库，并选择状态
          var orders = new Array();
          orders = JSON.parse(localStorage.getItem("newOrders"));
          if(orders.length>0&&orders[0].orderId==undefined){
              var json={list:orders}

              This.jqueryAjax("http://localhost:8081/order/saveOrders",orders,function(obj){
                    var ids = obj.data;
                    This.jqueryAjax("http://localhost:8081/order/getOrdersVoByOrderIds",ids,function(obj){
                        This.newOrders=obj.data;
                        localStorage.setItem("newOrders",JSON.stringify(This.newOrders));
                    })
              });

              This.orders=This.newOrders.concat(This.onlineOrders);
          }else{
            This.orders=This.onlineOrders;
          }

          This.$nextTick(() => {
            This.orders.forEach(row => {
              for(var i=0;i<This.newOrders.length;i++){
                if(row.orderId==This.newOrders[i].orderId){
                  This.$refs.multipleTable.toggleRowSelection(row)
                }
              }
            });
          }) 
          


        });

     
      
      This.requestPost("/user/selectUserPosition",null,function(){
        This.allPosition = This.mydata; 
      },null)

    },
    alertMessage:function(This,type,message){
      if(type == 0){
        type= 'success';
      }else if(type == 1){
        type= 'warning';
      }else if(type == 2){
        type= 'info';
      }else{
        type='info';
      }
      This.$message({
          message: message,
          type: type
      });

    },
    submitOrders:function(){
      var This = this;
      var ids="";
      for(var i=0;i<This.multipleSelection.length;i++){
        ids=ids+This.multipleSelection[i].orderId+",";
      }
      if(This.orders.length==0){
        This.alertMessage(This,2,"暂无订单，无法结算")
      }else if(This.selectPostion==null||This.selectPostion==''||This.selectPostion==undefined){
        This.alertMessage(This,2,"请选择发货地址")
      } else{
        if(This.multipleSelection.length>0){
          window.location.href="http://localhost:8081/alipay"+"?ids="+ ids+"&&position="+This.selectPostion; 
        }else{
          This.alertMessage(This,2,"请选择您想要结算的订单")
        }
      }

        
      
      
    },
    toggleSelection(rows) {
      var This=this;
        if (rows) {
          This.orders.forEach(row => {this.$refs.multipleTable.toggleRowSelection(row)});
        } else {
          this.$refs.multipleTable.clearSelection();
        }
      },
      handleSelectionChange(val) {
        this.multipleSelection = val;
      },
      handleDelete(index, row) {
        var obj=row;
        var This = this;
        if(confirm('确定要删除吗')==true){
          /**区分登录和未登录 */
          This.requestPost('/user/getLoginUser',null,function(){
            This.orders.splice(index,1);
            var newOrders = JSON.parse(localStorage.getItem("newOrders"));
            for(var i=0;i<newOrders.length;i++){
              if(newOrders[i].orderId==row.orderId){
                newOrders.splice(i,1);
              }
            }

            localStorage.setItem("newOrders",JSON.stringify(newOrders));
            This.requestPost("/order/destructionOrders",{orderId:row.orderId},null,null)
              
          },null);
          
        }

        

      },
      handleDeleteSelect(){
        var This=this;
        
        if(confirm('确定要删除吗')==true){
          var deleteId=new Array();
          for(var j=0;j<This.multipleSelection.length;j++){
            for(var i=0;i<This.orders.length;i++){
              if(This.multipleSelection[j].orderId==This.orders[i].orderId){
                This.orders.splice(i,1);
                deleteId.push(This.multipleSelection[j].orderId);
              }
            }
          }

          var newOrders = JSON.parse(localStorage.getItem("newOrders"));
          for(var i=0;i<deleteId.length;i++){
            var oId = deleteId[i]
            for(var j=0;j<newOrders.length;j++){
              if(newOrders[j].orderId==oId){
                newOrders.splice(j,1);
              }
            }

            This.requestPost("/order/destructionOrders",{orderId:oId},null,null)
          }
          localStorage.setItem("newOrders",JSON.stringify(newOrders));


          

          
        }
        

    },
    
},
    created:function(){
      this.initData();
    }
}

</script>

<style scoped>
.GoodsDetail .middle{
    width: 1200px;
    min-height: 100px;
    margin: 0 auto;;
}


.el-table--border, .el-table--group{
  border: none;
}
</style>